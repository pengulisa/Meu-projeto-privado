...
function updateDate() {
  const now = new Date();
  document.getElementById('dateDisplay').textContent = now.toLocaleDateString('pt-BR', {weekday:'short', day:'2-digit', month:'short', year:'numeric'});
}
...
function adicionarProduto() {
  ...
  ['prodNome','prodCategoria','prodCusto','prodVenda','prodEstoque'].forEach(id => document.getElementById(id).value = '');
}
...
function renderProdutos(filtro = '') {
  ...
  if (!lista.length) {
    tbody.innerHTML = '<tr><td colspan="7"><div class="empty-state"><div>📦</div>Nenhum produto cadastrado ainda.</div></td></tr>';
    return;
  }
...
function popularSelectProdutos() {
  ...
  sel.innerHTML = '<option value="">Selecionar...</option>' + 
    produtos.map(p => `<option value="${p.id}">${p.nome} — ${fmt(p.venda)}</option>`).join('');
...
function renderVendas() {
  ...
  if (!vendas.length) { 
    tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state"><div>🛒</div>Nenhuma venda registrada.</div></td></tr>'; 
    return; 
  }
...
function registrarPerda() {
  ...
  ['perdaDesc','perdaValor','perdaObs'].forEach(id => document.getElementById(id).value = '');
}

function renderPerdas() {
  const tbody = document.getElementById('tabelaPerdas');
  if (!perdas.length) { 
    tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state"><div>📉</div>Nenhuma perda registrada.</div></td></tr>'; 
    return; 
  }
...
// ---- HISTÓRICO ----
let historicoDias = JSON.parse(localStorage.getItem('loja_historico_dias') || '[]');
...
function renderHistorico() {
  ...
  historicoDias.forEach(dia => {
    dia.vendas.forEach(v => rows.push({ 
      data: v.data, 
      tipo: 'VENDA', 
      desc: `${v.qtd}x ${v.produto}`, 
      valor: v.total, 
    ...
    dia.perdas.forEach(p => rows.push({ 
      data: p.data, 
      tipo: 'PERDA', 
      desc: p.desc, 
      valor: p.valor, 
  ...
  const todos = [...hoje, ...rows].sort((a,b) => new Date(b.data) - new Date(a.data));

  if (!todos.length) { 
    tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state"><div>📋</div>Nenhuma movimentação registrada.</div></td></tr>'; 
    return; 
  }
  
  tbody.innerHTML = todos.map(t => `<tr>
...
function exportarCSV() {
  ...
  // dia atual
  vendas.forEach(v => rows.push([fmtDate(v.data),'VENDA',`${v.qtd}x ${v.produto}`,v.total.toFixed(2),'Em aberto']));
  perdas.forEach(p => rows.push([fmtDate(p.data),'PERDA',p.desc,p.valor.toFixed(2),'Em aberto']));
  
  // dias fechados
  historicoDias.forEach(dia => {
    dia.vendas.forEach(v => rows.push([fmtDate(v.data),'VENDA',`${v.qtd}x ${v.produto}`,v.total.toFixed(2),'Fechado']));
    dia.perdas.forEach(p => rows.push([fmtDate(p.data),'PERDA',p.desc,p.valor.toFixed(2),'Fechado']));
  });
...
// ---- ZERAR TUDO ----
function zerarTudo() {
  if (!confirm('⚠️ Tem certeza que quer ZERAR TUDO?\nProdutos, vendas, perdas e histórico serão apagados permanentemente!')) return;
  if (!confirm('🚨 Última confirmação! Isso não pode ser desfeito. Continuar?')) return;
